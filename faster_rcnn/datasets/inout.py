...
class inout(imdb):
    def __init__(self, image_set, encoding, devkit_path=None):
        imdb.__init__(self, 'inout_' + image_set+'_'+encoding)
        ...
        assert os.path.exists(self._devkit_path), 'InOut path does not exist: {}'.format(self._devkit_path)
    ...
    def image_path_from_index(self, index):
        ...
        :return filepath
        """
        image_path = os.path.join(self._data_path, self.encoding, index + self._image_ext)
        assert os.path.exists(image_path), 'Path to image does not exist: {}'.format(image_path)
        return image_path
    ...
    def _get_default_path(self):
        """
        Return the default path where InOut is expected to be installed.
        """
    ...
    #     This function loads/saves from/to a cache file to speed up future calls.
    #     """
    #     cache_file = os.path.join(self._devkit_path, 'faster_rcnn', 'cache',self.name + '_selective_search_roidb.pkl')
    #
    #     if os.path.exists(cache_file):
    #         with open(cache_file, 'rb') as fid:
    ...
    #     filename = os.path.abspath(os.path.join(self._data_path,'faster_rcnn','selective_search_data',self.name + '.mat'))
    #     assert os.path.exists(filename), \
    ...
    def _remove_empty_samples(self):
        ...
        for i in range(len(self._image_index)-1, -1, -1):
            ...
            filename = os.path.join(self._data_path, 'Annotations_pv', index + '.xml')
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self._devkit_path, 'faster_rcnn', 'results')
